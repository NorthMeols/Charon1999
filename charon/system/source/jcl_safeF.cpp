...
// Arg 1 Program to run JCL for
//     2 Root of playpen
//     3 Directory in playpen to do this in
//     4 File to run
//     5 Data
//     6 Output of run
...
{
  srand( (int) time(0) );
  int randomPID = rand()%(65530-BASE_PID)+BASE_PID;   // PID to run as
  ...
  if ( argc >= 5 )
  {
    ...
    if ( result != 0 )
    {
      std::cout << "Security failure: Can not chdir to " << argv[1] << "\n";
      return -1;
    }
